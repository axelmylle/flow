...
import type { UpdateTransactionValues } from "@/actions/transactions/schema";
...
import { FreelancerDetailSheet } from "@/components/freelancers/profile/sheets/freelancer-detail-sheet";
...
export function DataTable<TData, TValue>({
  ...
  const setOpen = (id?: string) => {
    if (id) {
      setParams({ type: "details", freelancerId: id });
    } else {
      setParams(null);
    }
  };
  ...
  const { setParams, freelancerId, type } = useFreelancerSearchParams();
  ...
  const selectedFreelancer = data.find(
    (invoice) => invoice?.id === freelancerId,
  );
  ...
  return (
    <div className="mb-8 relative">
      ...
      <FreelancerDetailSheet
        ...
        user={selectedFreelancer?.user}
        isOpen={type === "details" && !!freelancerId}
        setOpen={setOpen}
      />
      <BottomBar
        show={showBottomBar}
